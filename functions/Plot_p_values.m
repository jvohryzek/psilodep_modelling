function [F1, F2, F3, sigC] = Plot_p_values(P_pval,rangeK,Kmeans_results)
...
for i=1:length(V_best)
    if V_best(i)>0
        handlebar(i) = barh(i,V_best(1,i).*(V_best(1,i)>0), 'BarWidth', 0.9);
        set(handlebar(i), 'FaceColor', [V_best(i) 0  0],'EdgeColor',[V_best(i) 0  0]);
    elseif V_best(i)<=0
        handlebar(i) = barh(i,V_best(1,i).*(V_best(1,i)<=0), 'BarWidth', 0.9);
        set(handlebar(i), 'FaceColor', [0 -V_best(i) 0],'EdgeColor',[0 -V_best(i) 0]);
    end
    hold on
end

% old version
% barh(V_best.*(V_best<=0),'FaceColor',[V(n) 0  0],'EdgeColor','none')
...
%% The FC pattern matrix (exlcuded for now)
% subplot(4,2,7)
% colormap(jet)
% FC_V=V_best'*V_best;  
% li=max(abs(FC_V(:)));
