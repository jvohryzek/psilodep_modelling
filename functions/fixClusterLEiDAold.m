function [PTRANSITION,Pstates] = fixClusterLEiDA(BOLD,numClusters,Center,TR)
...
for t=1:tAll
    for j = 1:numClusters
     di(j) = sqrt(sum(leadingEigV1(t,:) - Center(j,:)).^2); % comparing clusters to the empirical data (sqeuclidean)
    end
    [aux indmin] = min(di);
    IDX(t) = indmin;
end

% OPTION 1: Fractional Occupancy
