function Video_Nodes_Cortex(Leading_Eig) %,IDX) 
...
for n=1:length(V)
    if V(n)>0
        h(n)=surf(x*a+scale*MNI_coord(n,2)+ori(1), y*a+scale*MNI_coord(n,1)+ori(2),z*a+scale*MNI_coord(n,3)+ori(3),'FaceColor',[V(n) 0 0],'EdgeColor','none','FaceAlpha',1);
  elseif V(n)<0
        h(n)=surf(x*a+scale*MNI_coord(n,2)+ori(1), y*a+scale*MNI_coord(n,1)+ori(2),z*a+scale*MNI_coord(n,3)+ori(3),'FaceColor',[0 0 abs(V(n))],'EdgeColor','none','FaceAlpha',.5);
    end
end
...
for t=1:length(V1_awake)
    ...
    for n=1:length(V)
        if V(n)>0
            set(h(n),'FaceColor',[V(n) 0 0]);
        elseif V(n)<0
            set(h(n),'FaceColor',[0 0 abs(V(n))]);
        end
    end
    ...
    if numel(n_strong)>1
        ...
        for a=1:numel(n_strong)
            ...
            for b=1:a
                ...
                ch(u)=plot3([c1(1) c2(1)],[c1(2) c2(2)],[c1(3) c2(3)],'Color','r'); %cmap(IDX(t),:));
                u=u+1;
            end
        end
    end
    
    % Other features of the image can be updated here for any timeseries
