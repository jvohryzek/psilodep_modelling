...
for n=1:length(V)
    if V(n)>0
        surf(x+MNI_coord(n,2)+ori(1), y+MNI_coord(n,1)+ori(2),z+MNI_coord(n,3)+ori(3),'FaceColor',[V(n) 0  0],'EdgeColor','none','FaceAlpha',0.7);
    elseif V(n)<=0
        surf(x+MNI_coord(n,2)+ori(1), y+MNI_coord(n,1)+ori(2),z+MNI_coord(n,3)+ori(3),'FaceColor',[0 -V(n) 0],'EdgeColor','none','FaceAlpha',0.7);
    end
end
